...
export function selectClientForTicket(id, nombre) {
  ...
  // NUEVO: Verificar garantías activas
...
export async function changeTicketStageAt(id, newStage) {
    ...
    if(t && t.stage !== newStage){
        ...
        // NUEVO: Si se entrega, iniciar cálculo de Garantía automáticamente
        if(newStage === 'entregado') {
            const diasGarantia = t.garantiaDias !== undefined ? t.garantiaDias : 30; // 30 días por defecto
...
export async function saveGarantiaDias() {
    ...
    // Si el equipo ya fue entregado, le recalculamos la fecha en base a hoy o a la fecha en que se entregó.
    // Para simplificar, si cambias los días, recalculamos desde la fecha actual para adelante.
    if(t.stage === 'entregado') {
        const fVence = new Date();
    ...
    try {
        ...
        toast('Error guardando garantía');
    }
}
